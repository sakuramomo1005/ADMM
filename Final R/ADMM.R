...
bi_ADMM_order = function(X, nu1, nu2, gamma_1, gamma_2, kk=5, phi=0.5,niter = 1000,tol = 0.1,output = 1){
  ...
  # 11/25 change
  # w <- dist_weight(t(X),phi/n, dist.type = "euclidean", p = 2 )
  # w_l <- knn_weights(w,k = kk,p)
  # w_l = w_l/sum(w_l)
  # w_l = w_l/sqrt(n)
  # 
  ...
  # 
  # u <- dist_weight(X,phi/p, dist.type = "euclidean", p = 2 )
  # u_k <- knn_weights(u,k = kk,n)
  # u_k = u_k/sum(u_k)
  # u_k = u_k/sqrt(p)
...
cluster_assign = function(X, result, method = 'ADMM'){
  phi = 0.5
